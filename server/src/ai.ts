 ...
 * 4. Return the new game state and shot result
 */
export async function processAITurn(
  state: GameState,
  aiPlayerId: string
): Promise<{ newState: GameState; result: ShotResultPayload }> {
  // 1. Simulated thinking delay
  const delay = AI_THINK_MIN_MS + Math.floor(Math.random() * AI_THINK_RANGE_MS);
  await new Promise<void>((resolve) => setTimeout(resolve, delay));

  // 2. Get AI decision (service or fallback)
  const coordinate = await getAIDecision(state, aiPlayerId);

  // 3. Apply the shot via domain logic
  return applyShot(state, aiPlayerId, coordinate);
}
